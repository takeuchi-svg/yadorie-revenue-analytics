...
export interface AuthOk {
  userId: string
  isAdmin: boolean
  facilities: string[] | null // null = 全施設可（admin）
}
...
export async function requireUser(req: NextRequest): Promise<AuthOk | AuthErr> {
  ...
  if (au?.role === 'admin') return { userId: user.id, isAdmin: true, facilities: null }
  // member（app_user 行なしも member 扱い＝フェイルクローズ）
  const { data: uf } = await admin.from('user_facility').select('facility').eq('user_id', user.id)
  return { userId: user.id, isAdmin: false, facilities: ((uf ?? []) as { facility: string }[]).map((r) => r.facility) }
}

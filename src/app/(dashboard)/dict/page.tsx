...
// 辞書。一般ユーザーは公開中のKPI辞書・用語集を閲覧。オーナーは編集（下書き→公開・履歴）。
import { useCallback, useEffect, useMemo, useState } from 'react'
import { supabase } from '@/lib/supabase/client'
import { useFacility } from '@/lib/facility-context'
import StructuredTab from '../knowledge/structured'
...
export default function DictPage() {
  ...
  useEffect(() => { if (!isOwner) load(); else setLoading(false) }, [load, isOwner])
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-1 mb-4">
        <button onClick={() => setTab('kpi')} className="px-4 py-1.5 rounded-md text-sm" style={{ background: tab === 'kpi' ? 'var(--accent)' : 'var(--surface2)', color: tab === 'kpi' ? '#fff' : 'var(--text-dim)' }}>KPI辞書</button>
        <button onClick={() => setTab('glossary')} className="px-4 py-1.5 rounded-md text-sm" style={{ background: tab === 'glossary' ? 'var(--accent)' : 'var(--surface2)', color: tab === 'glossary' ? '#fff' : 'var(--text-dim)' }}>用語集</button>
      </div>

      {isOwner ? (
        <StructuredTab kind={tab === 'kpi' ? 'kpi' : 'glossary'} />
      ) : loading ? (
        <p className="text-sm" style={{ color: 'var(--text-dim)' }}>読み込み中...</p>
      ) : (
        <>
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="キーワードで絞り込み" className="field px-3 py-1.5 text-sm mb-3" style={{ minWidth: 240 }} />
          {tab === 'kpi' ? (

...
本システムは**旅館運営システム「YADORIE Core」**（旧称: YADORIE Revenue Analytics）。
**思想と人格の正 = `/docs/YADORIE_Core_コンセプト定義書.md`**。UIコピー・AI生成文・メッセージの
トーンはAI人格「灯（あかり）＝若女将」の語り口に従う（照らすが裁かない。悪い数字も次の一手とセット。
数値・表は無機質に正確でよく、人格は「語り」の部分のみ）。タグライン「宿の数だけ、ストーリー。」。

**着手前に `/docs/00_ドキュメント地図.md` を読む**（どれが正本かの索引）。

- DB全体（テーブル/ビュー/キー/前提）の正本 = `/docs/データベース全体設計.md`
- 実行するDDL = `/scripts/sql/*.sql`（冪等・SQL Editorで手動適用）
- 運用（月次取込・DDL適用・権限） = `/docs/運用手順書.md`
- UI/要件/各機能の背景 = 各docs（ただし古い記述は冒頭の「⚠️ 更新」注記に従う）

実装が設計と乖離したら、CLAUDE.mdに書くだけで終わらせず**正本docへ還流**する（as-buildルール）。
